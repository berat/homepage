import { ProcessedBlock, RichTextContent } from "@/lib/notion/types";
    ...
    if (link) {
      ...
      element = (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="underline text-blue-600 transition"
          title={link}
        >
  ...
  switch (block.type) {
    case "quote":
      return (
        <blockquote
          key={block.id}
          className=" border-primary rounded-r-lg py-5 px-5 text-slate-600 border-l-3 pl-5 leading-[1.7]"
        >
          {renderRichText(block.content)}
        </blockquote>
      );
    case "paragraph":
      return (
        <p key={block.id} className="text-primary leading-[1.8]">
          {renderRichText(block.content)}
        </p>
      );
    case "heading_1":
      return (
        <h1
          key={block.id}
          className="text-primary mt-6 font-sans text-3xl font-bold"
        >
          {renderRichText(block.content)}
        </h1>
      );
    case "heading_2":
      return (
        <h2
          key={block.id}
          className="text-primary mt-6 font-sans text-2xl font-semibold"
        >
          {renderRichText(block.content)}
        </h2>
      );
    case "heading_3":
      return (
        <h3
          key={block.id}
          className="text-primary mt-5 font-sans text-xl font-semibold"
        >
          {renderRichText(block.content)}
        </h3>
      );
    ...
    case "divider":
      return <hr key={block.id} className="border-gray-200 my-6 border-t" />;
    case "image":
      return (
        <div key={block.id}>
    ...
    case "video":
      const youtubeId = getYouTubeVideoId(block.content[0].text.content);

      if (youtubeId === null) return null;

      return (
        <div className="youtube-embed" key={youtubeId}>
          <YouTubeEmbed videoid={youtubeId} params="controls=0" />
        </div>
      );
    case "embed":
      const tweetId = getTweetId(block.content[0].text.content);
    ...
    if (block.type === "numbered_list_item" && !isPreview) {
      ...
      result.push(
        <ol
          ...
          className="ml-6 list-decimal space-y-1.5"
        >
          {listItems}
        </ol>
      );
